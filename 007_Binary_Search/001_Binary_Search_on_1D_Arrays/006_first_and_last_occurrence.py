"""
INTUITION:
Given a sorted array nums and a target element, we have to find the first and last occurrence of target in nums.
...
If they do, we return the index, if they dont, we return [-1, -1].
...
3. Check if lower bound index does lie within the nums length, and lower bound element is equal to target-
    1. If false, return [-1, -1]
    2. If true, return [lower bound index, upper bound index - 1]
...
Brute force includes linearly traversing through the array to find the first and last occurrence of the element, which takes O(n) time, and does not use the property of the array being sorted.
This approach is optimal since we find the first and last occurrence of the target element in O(log n) time.


WHAT I'D SAY IN AN INTERVIEW:
We are given a sorted array nums and a target element, and we have to find the first and the last occurrence of the target.
This can be done by first find the lower bound index of the target in nums, and then finding the upper bound index of target in nums. Lower bound is the smallest index where nums[index] >= target, and upper bound is the smallest index where nums[index] > target. Once we have the lower and upper bound index, we can check that the lower bound index is not equal to the hypothetical index, or lower bound element does equal to the target. If both are true, we return [lower bound index, upper bound index - 1], else we return [-1, -1].
