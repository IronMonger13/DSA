"""
INTUITION:
Given a sorted array nums and a target, find the lower bound. Lower bound is the smallest index where nums[lower bound] >= target.
This can be found using binary search. We eliminate the search space where elements are lower than target.
...
nums = [1, 2, 2, 3]
target = 2
print(Solution().lb(nums, target))

"""
TIME COMPLEXITY:
O(log n) - we keep dividing the search space to find lower bound.
...
Brute force includes performing linear search to find the lower bound element, which takes O(n) time.
This approach is optimal since we keep dividing the search space to find lower bound, which takes O(log n) time.


WHAT I'D SAY IN AN INTERVIEW:
We are given a sorted array nums and a target. We need to find the lower bound. Lower bound is the smallest index where nums[lower bound] >= target.
